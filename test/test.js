...
describe("Cool Token Contract Testing", function () {
  ...
  before(async () => {
    ...
    const coolToken = await hre.ethers.getContractFactory("CoolToken")
    CoolTokenContract = await coolToken.deploy(supply, airdropAmount, timestamp)
    await CoolTokenContract.deployed()
  })
  ...
  it("Reads values minted from deployment", async function () {
    ...
    /* total supply */
    const totalSupply = await CoolTokenContract.totalSupply()
    ...
    /* treasury balance */
    const treasBalance = await CoolTokenContract.balanceOf(treasury.address)
    ...
    /* contract balance */
    const contractBalance = await CoolTokenContract.balanceOf(CoolTokenContract.address)
  ...
  it("Sets the merkle root, should not allow to set again", async function () {
    ...
    await CoolTokenContract.setMerkleRoot(root)
    expect(CoolTokenContract.setMerkleRoot(root)).to.be.reverted;
    contractRoot = await CoolTokenContract.merkleRoot();
  ...
  it("Should allow a user to claim tokens", async function () {
    ...
    await CoolTokenContract.connect(claimer1).claimTokens(500, proof)
    const airdropPersonalAmount = ethers.utils.parseEther("500")

    /* Get claimer balance */
    let claimerBalance = await CoolTokenContract.balanceOf(claimer1.address)
  ...
  it("Should reject a false proof, using claimer1 address for claimer2", async () => {
    ...
    try {
      await CoolTokenContract.connect(claimer2).claimTokens(proof)
      error = true
    } catch (err) {}
  ...
  it("Should not allow the second account to claim a wrong amount of tokens", async () => {
    ...
    try {
      await CoolTokenContract.connect(claimer2).claimTokens(800, proof)
      error = true
    } catch (err) {}
  ...
  it("Should allow the second account to claim the right amount of tokens", async () => {
    ...
    await CoolTokenContract.connect(claimer2).claimTokens(300, proof)
    ...
    /* Get claimer balance */
    let claimerBalance = await CoolTokenContract.balanceOf(claimer2.address)
    expect(claimerBalance).to.equal(airdropPersonalAmount);
    console.log('Claimer balance: ', ethers.utils.formatEther(claimerBalance))
  ...
  it("Should allow the treasury to mint", async function () {
    ...
    /* treasury balance */
    const balance = await CoolTokenContract.balanceOf(treasury.address)
    console.log('Treasury balance:', ethers.utils.formatEther(balance))

    /* mint more tokens */
    await CoolTokenContract.connect(treasury).mint(1_000_000)
    const millionEther = ethers.utils.parseEther("1000000")
    const expectedValueAfter = balance.add(millionEther);
    expect(await CoolTokenContract.balanceOf(treasury.address)).to.equal(expectedValueAfter)

    let treasuryBalance = await CoolTokenContract.balanceOf(treasury.address)
  ...
  it("Should not allow the treasury to mint after minting is disabled", async function () {
    let error
    await CoolTokenContract.connect(treasury).mint(1_000_000)  
    await CoolTokenContract.connect(treasury).disableMinting()  
    try {
      await CoolTokenContract.connect(treasury).mint(1_000_000)  
      error = true
    } catch (err) {}
